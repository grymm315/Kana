...
class PhonemeViewController: UIViewController, OEEventsObserverDelegate
{
    ...
    override func viewWillAppear(_ animated: Bool)
    {
        ...
        DispatchQueue.main.async {
            for (index, letter) in self.alphabet.enumerated(){
                self.view.addSubview(kanaBttn(str: letter, soundsLike: self.words[index]))
            }
        }
    ...
    func addKana(letter: String)-> kanaBttn {
        var button: kanaBttn
        button = kanaBttn(str: letter, soundsLike: "ah") //frame: CGRect(x: 10, y: 40, width: 50, height: 50))
    ...
    func pocketsphinxDidReceiveHypothesis(_ hypothesis: String!, recognitionScore: String!, utteranceID: String!) { // Something was heard
        ...
        NotificationCenter.default.post(.init(name: Notification.Name(rawValue: hypothesis)))
    }
